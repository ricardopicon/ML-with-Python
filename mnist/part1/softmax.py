...
def compute_probabilities(X, theta, temp_parameter):
    ...
    temp = ((theta.dot(X.transpose()))/temp_parameter)
    c = np.amax(temp,axis=1)
    ...
    #     ctc = theta.shape[0]-1*i*X/temp_parameter
    #     np.exp(theta[i]*(X/temp_parameter)-ctc)
...
def compute_cost_function(X, Y, theta, lambda_factor, temp_parameter):
    ...
    """
    theta2 = sum(sum(theta.dot(theta.transpose())))
    theta2*=lambda_factor/2
    n = X.shape[0]
    ...
    for i in range(n):
        for j in range(theta.shape[0]):
            if Y[i]==j:
                sums += np.nan_to_num(np.log(probs[i][j]),neginf=0)
    sums *= -1/n
    return theta2+sums
    raise NotImplementedError
